...
class cryptoJS{
	...
	req_sync(){
		return socketctl.enviar({tipo: "req-sync", msg: this.creds}).then(a=>this.desencriptar(a)).then(b=>{
			if(b.data.length<1) throw new Error("Parece que no tienes ninguna base de datos asociada a la cuenta");
	...
	decidir_sync(a){
		return dbjs.backup({usuario: this.creds.usuario}).then(b=>{
			return socketctl.enviar({tipo: "decidir_sync", msg: {usuario: this.creds.usuario, db: sha256(btoa(JSON.stringify(b))), fecha: a.fecha}}).then(c=>{
				if(c==false){
					return false;
				}
				if(c=="servidor"){
