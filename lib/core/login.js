...
$("#login-login").on("click", ()=>{
	...
	if($("#login-login").attr("data-login")=="local"){
		...
		cryptoJS.authOnline(credenciales).then(a=>{
			console.log(a);
			if(a){
				vex.dialog.alert("Hemos descargado tu información correctamente. En un momento estará en tu cuenta");
			}else{
				vex.dialog.alert("No hemos podido autenticarte en el servidor");
				$("#login-usuario").change();
			}
		}).catch(e=>{
			console.error(e);
			if(e.toString().length<4){
				vex.dialog.alert("No sabemos cuál ha sido el fallo pero nos informan de que no ha sido posible iniciar sesión");
			}else{
...
$("#login-registro").on("click", ()=>{
	...
	if(credenciales.contraseña.length<1){
		vex.dialog.alert({
			message: "Debes de introducir una contraseña",
			callback: $("#login-usuario").change()
		});
		return;
	}
