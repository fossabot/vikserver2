...
if(typeof window!=="undefined"){
    load(["core/crypto.js", "socket_io"]).then(x=>{
        window.msgs=[];
        window.socketctl=new Socket();
    });
}else{
    load("socket_io").then(x=>{
...
class Socket{
	constructor(a){
		function conn(_this){
			_this.socket= io("https://vikserver-backend.herokuapp.com");
	...
	enviarEncriptado(a){
		return cryptoJS.encriptar(a.msg).then(b=>{
			return this.enviar({tipo: a.tipo || "crypt-msg", msg: b.data, timeo:a.timeo});
		});
	}
	terminar(){
