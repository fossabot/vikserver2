...
class cryptoJS{
	...
	authOnline(a){
		...
		this.importarOnline(a).then(()=>{
			return socketctl.enviarEncriptado({tipo: "login", msg: {creds: a, key: this.pgp.pública}}).then(b=>{
				if(b==false) throw new Error("Las credenciales de acceso no son correctas");
				return this.desencriptar(b).then(c=>{
					return dbjs.insertar({Usuario: this.creds.usuario, db: c.data});
				});
			});
		});
	}
