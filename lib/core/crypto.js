...
class cryptoJS{
	...
	authOnline(a){
		...
		return this.importarOnline(a).then(()=>{
			this.iniciarCredenciales(a);
			return socketctl.enviarEncriptado({tipo: "login", msg: {creds: a, key: this.pgpArmor.pública}}).then(b=>{
				if(b==false) throw new Error("Las credenciales de acceso no son correctas");
				return this.req_sync().then(c=>{
					return dbjs.restaurar(c.data);
				});
			}).catch(e=>{
				console.error(e);
				throw new Error("Fallo al enviar las credenciales al servidor");
			});
		}).catch(e=>{
			console.error(e);
